                                          ...
                                          std::vector<std::string> &types, std::chrono::nanoseconds timeout )
{
  ...
  if ( has_topic( node, topic, types )) return true;
  ...
  std::chrono::nanoseconds time_to_wait =
    timeout > std::chrono::nanoseconds( 0 ) ? timeout - (std::chrono::steady_clock::now() - start)
...
{
  std::string resolved_topic = rclcpp::extend_name_with_sub_namespace( topic, node.get_sub_namespace());
