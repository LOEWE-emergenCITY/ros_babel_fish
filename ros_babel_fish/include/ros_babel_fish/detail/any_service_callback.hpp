...
public:
  ...
  //! @return True if the callback filled the response and it should be sent immediately, false if
  //!   the callback deferred the response (a defer variant), in which case the caller must not send.
  bool dispatch( const std::shared_ptr<BabelFishService> &service_handle,
