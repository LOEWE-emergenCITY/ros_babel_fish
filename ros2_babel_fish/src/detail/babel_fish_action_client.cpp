...
Client<ros2_babel_fish::impl::BabelFishAction>::Client( rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node_base,
                                                        rclcpp::node_interfaces::NodeGraphInterface::SharedPtr node_graph,
                                                        rclcpp::node_interfaces::NodeLoggingInterface::SharedPtr node_logging,
                                                        const std::string &action_name,
                                                        ros2_babel_fish::ActionTypeSupport::ConstSharedPtr type_support,
                                                        const rcl_action_client_options_t &client_options )
  : ClientBase(
  node_base, node_graph, node_logging, action_name,
  &type_support->type_support_handle,
  client_options ), type_support_( std::move( type_support ))
{
}
...
CompoundMessage Client<impl::BabelFishAction>::create_goal() const
{
  ...
  size_t index = std::find_if( introspection->members_, introspection->members_ + introspection->member_count_,
...
std::shared_future<rclcpp_action::ClientGoalHandle<ros2_babel_fish::impl::BabelFishAction>::SharedPtr>
                                                                 ...
                                                                 const SendGoalOptions &options )
{
  ...
  // Clean old goal handles
  {
    ...
    {
      if ( !it->second.lock()) it = goal_handles_.erase( it );
      else ++it;
    }
  }
  return future;
...
std::shared_future<rclcpp_action::ClientGoalHandle<ros2_babel_fish::impl::BabelFishAction>::WrappedResult>
...
Client<impl::BabelFishAction>::async_cancel_goal( GoalHandle::SharedPtr goal_handle, CancelCallback cancel_callback )
{
  std::lock_guard<std::mutex> lock( goal_handles_mutex_ );
  ...
  auto &request_uuid = cancel_request["goal_info"]["goal_id"]["uuid"].as<FixedLengthArrayMessage<uint8_t >>();
  for ( size_t i = 0; i < uuid.size(); ++i ) request_uuid[i] = uuid[i];
...
std::shared_future<ros2_babel_fish::CompoundMessage>
Client<impl::BabelFishAction>::async_cancel_goals_before( const rclcpp::Time &stamp, CancelCallback cancel_callback )
{
  CompoundMessage cancel_request( type_support_->cancel_service_type_support->request());
  cancel_request["goal_info"]["goal_id"]["uuid"].as<FixedLengthArrayMessage<uint8_t>>().fill( 0u );
...
std::shared_future<ros2_babel_fish::CompoundMessage>
Client<impl::BabelFishAction>::async_cancel( CompoundMessage cancel_request, CancelCallback cancel_callback )
{
...
void Client<impl::BabelFishAction>::handle_feedback_message( std::shared_ptr<void> message )
{
  ...
  GoalUUID goal_id;
  const auto &feedback_goal_id = feedback_message["goal_id"]["uuid"].as<FixedLengthArrayMessage<uint8_t>>();
  ...
  goal_handle->call_feedback_callback(
    goal_handle,
    CompoundMessage::make_shared( feedback_message["feedback"].as<CompoundMessage>())
  );
}
...
void Client<impl::BabelFishAction>::handle_status_message( std::shared_ptr<void> message )
{
  ...
  for ( size_t i = 0; i < status_list.size(); ++i )
  {
    ...
    GoalUUID goal_id;
    const auto &status_goal_id = status["goal_info"]["goal_id"]["uuid"].as<FixedLengthArrayMessage<uint8_t>>();
...
void Client<ros2_babel_fish::impl::BabelFishAction>::make_result_aware( GoalHandle::SharedPtr goal_handle )
{
  ...
  const auto &uuid = goal_handle->get_goal_id();
  auto &request_uuid = goal_result_request["goal_id"]["uuid"].as<FixedLengthArrayMessage<uint8_t>>();
  for ( size_t i = 0; i < uuid.size(); ++i ) request_uuid[i] = uuid[i];
  try
  {
    this->send_result_request(
      goal_result_request.type_erased_message(),
      [ goal_handle, this ]( std::shared_ptr<void> data )
      {
        CompoundMessage response( type_support_->result_service_type_support->response(), data );
        WrappedResult wrapped_result;
        wrapped_result.result = CompoundMessage::make_shared( response["result"].as<CompoundMessage>());
