  ...
  ArrayMessageBase &operator=( const ArrayMessageBase &other )
  {
    ...
    if ( elementType() != other.elementType())
      throw BabelFishException(
        std::string( "Incompatible array types! (" ) + std::to_string( elementType()) + " vs " +
...
protected:
  ...
  static_assert((FIXED_LENGTH && BOUNDED) || !FIXED_LENGTH, "Fixed length can only be true if bounded is also true!" );
...
public:
  ...
  explicit ArrayMessage_( MessageMemberIntrospection member,
  ...
  {
    using Container = typename std::conditional<FIXED_LENGTH, std::array<T, 987654321000>, std::vector<T>>::type;
  ...
  operator[]( size_t index )
  {
    // Need to specialize for bool because std::vector<bool> is specialized and uses one bit per bool, hence, you can
    // not return a bool reference but need to use std::_Bit_reference
    if ( index >= size()) throw std::out_of_range( "Index was out of range of array!" );
  ...
  ConstReturnType operator[]( size_t index ) const
  {
    ...
    return *reinterpret_cast<const ConstReturnType *>(member_->get_const_function( data_.get(), index ));
  }
  ...
  void resize( size_t length )
  {
    ...
    if ( member_->resize_function == nullptr ) reinterpret_cast<std::vector<T> *>(data_.get())->resize( length );
    else member_->resize_function( data_.get(), length );
  }
...
protected:
  ...
  void _assignImpl( const ArrayMessageBase &other )
  {
    ...
    {
      throw std::out_of_range( "Can not assign ArrayMessage as it exceeds the maximum size of this ArrayMessage!" );
    }
    if ( !FIXED_LENGTH ) resize( other.size());
    for ( size_t index = 0; index < other.size(); ++index )
      at( index ) = other_typed.at( index );
  }
  ...
  void _assignImpl( const ArrayMessageBase &other )
  {
    ...
    if ( BOUNDED && other.size() > maxSize())
    {
      throw std::out_of_range(
        "Can not assign CompoundArrayMessage as it exceeds the maximum size of this CompoundArrayMessage!" );
    }
    if ( !FIXED_LENGTH ) resize( other.size());
    for ( size_t index = 0; index < other.size(); ++index )
      at( index ) = other_typed.at( index );
