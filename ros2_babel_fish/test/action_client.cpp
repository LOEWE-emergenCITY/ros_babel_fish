...
using namespace ros2_babel_fish_test_msgs;

rclcpp::Node::SharedPtr node;

::testing::AssertionResult Equal( const rosidl_message_type_support_t &a, const rosidl_message_type_support_t &b )
{
  if ( a.data != b.data ) return ::testing::AssertionFailure() << "Type support maps are not equal!";
  return ::testing::AssertionSuccess();
}

::testing::AssertionResult Equal( const rosidl_action_type_support_t &a, const rosidl_action_type_support_t &b )
{
...
{
  BabelFish fish;
  ActionTypeSupport::ConstSharedPtr type_support = fish.get_action_type_support(
    "ros2_babel_fish_test_msgs/action/SimpleTest" );
  ASSERT_NE( type_support, nullptr );


  const rosidl_action_type_support_t *ts = rosidl_typesupport_cpp::get_action_type_support_handle<ros2_babel_fish_test_msgs::action::SimpleTest>();
//  EXPECT_EQ(type_support->type_support_handle, *ts);
  EXPECT_NE( ts, nullptr );
  EXPECT_TRUE( Equal(type_support->type_support_handle, *ts));

  type_support = fish.get_action_type_support(
    "action_tutorials_interfaces/action/Fibonacci" );
  ASSERT_NE( type_support, nullptr );

  const auto *ts_map = static_cast<const type_support_map_t *>(ts->status_message_type_support->data);
  const auto *map = static_cast<const type_support_map_t *>(type_support->status_message_type_support->type_support_handle.data);
...
//  Message::SharedPtr goal = fish.createMessage( "ros2_babel_fish_test_msgs/SimpleTestGoal" );
//  (*goal)["goal"] = 5;
...
//  goal = fish.createMessage( "ros2_babel_fish_test_msgs/SimpleTestGoal" );
//  (*goal)["goal"] = 20;
