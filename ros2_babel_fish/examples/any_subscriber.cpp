...
void dumpMessageContent( const ros2_babel_fish::Message &message, const std::string &prefix = "" );
...
public:
  ...
  void init()
  {
    fish = ros2_babel_fish::BabelFish::make_shared();
    subscription_ = fish->create_subscription( *this, topic_, 10,
                                               [ this ]( ros2_babel_fish::CompoundMessage::SharedPtr msg )
                                               {
                                                 topic_callback( msg );
                                               } );
  }
...
private:
  void topic_callback( const ros2_babel_fish::CompoundMessage::SharedPtr &msg ) const
  {
    if ( msg == nullptr )
    {
  ...
  std::string topic_;
  ros2_babel_fish::BabelFish::SharedPtr fish;
...
template<typename T>
void printValueMessageToStdOut( const ros2_babel_fish::Message &val )
{
...
void printArray(
  const ros2_babel_fish::CompoundArrayMessage_<BOUNDED, FIXED_LENGTH> &message,
...
template<typename T>
void printArrayMessage( const ros2_babel_fish::ArrayMessageBase &message, const std::string &prefix )
{
  printArray( message.as<ros2_babel_fish::ArrayMessage<T>>(), prefix );
}
  ...
  else
  {
    switch ( message.type())
    {
      ...
      default:
        ...
        // Or use one of the invoke methods in template_helpers.hpp for a better C++ solution that can also handle
        // functors with a return value.
    }
  }
}
